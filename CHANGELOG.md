...
- One home per shared definition, marked for the family drift gate: Sections 3.1 and 3.1.1
  are the home of the `did:opena2a` string form, now stating that signed artifacts carry
  exactly one form (the unescaped form) and that verifiers compare after the Section 3.3
  normalization and never normalize before signature verification; Section 3.2's table is
  the shared resource-type registry, from which `registries/resource-types.json` is
  generated by `scripts/gen_registries.py` and checked in CI (`registries` workflow).
